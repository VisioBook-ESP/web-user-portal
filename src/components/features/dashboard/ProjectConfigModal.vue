...
onMounted(() => {
  ...
  if (props.existingConfig) {
    if (props.existingConfig.style) config.value.style = props.existingConfig.style;
    if (props.existingConfig.audioVoice) config.value.audioVoice = props.existingConfig.audioVoice;
    if (props.existingConfig.quality) config.value.quality = props.existingConfig.quality;
  }
...
const styleOptions = [
  ...
  { value: 'cartoon', label: 'Cartoon', description: 'Western animation style' },
  { value: 'anime', label: 'Manga', description: 'Japanese manga style' },
  { value: 'comic', label: 'Watercolor', description: 'Watercolor painting effect' },
];
...
const isFormValid = computed(() => {
  return config.value.style && config.value.audioVoice && isOcrComplete.value && ocrText.value.trim().length > 0;
});
...
const handleGenerate = async () => {
  ...
  try {
    ...
    uiStore.showError(error.message || 'Configuration failed. Please try again.');
  } finally {
...
<template>
  <div class="config-modal-overlay" :style="gradientStyle" @click.self="closeModal">
    <div class="config-modal">
      ...
      <!-- Title -->
      <h1 class="modal-title">{{ isEditMode ? 'Edit Your VisioBook' : 'Configure Your VisioBook' }}</h1>
      ...
      <div class="ocr-section">
        ...
        <div v-if="isProcessingOcr" class="ocr-processing">
          ...
          <div class="progress-bar-container">
            <div class="progress-bar-fill" :style="{ width: `${ocrProgress}%` }"></div>
          </div>
        ...
        <div v-else class="ocr-result">
          ...
          <div class="ocr-info">
            ...
            <span v-if="isOcrComplete" class="edit-hint">You can edit the text if needed</span>
          </div>
        </div>
      </div>
      ...
      <div class="config-form" :class="{ disabled: !isOcrComplete }">
        ...
        <div class="config-section">
          ...
          <div class="voice-options">
            <div
              ...
              @click="config.audioVoice = voice.value as ProjectConfig['audioVoice']"
            >
        ...
        >
          ...
          <span v-else>{{ isEditMode ? 'Save Changes' : 'Generate VisioBook' }}</span>
        </button>
